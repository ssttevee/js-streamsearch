...
function test(
	...
	t.same(
		outchunks.map((chunks) =>
			chunks.map((chunk) => u8.toString(chunk)).join(""),
		),
		expected,
	);
...
function suite(t: Test, name: string, split: (s: string) => string[]): void {
	t.test(name, (t: Test): void => {
		t.test("1 character needle", (t: Test): void => {
			t.test(
				"should return the original payload if the needle cannot be found",
				(t: Test) => {
			...
			t.test(
				"should return the payload split by the needle",
				(t: Test): void => {
		...
		t.test("2 (different) character needle", (t: Test): void => {
			t.test(
				"should return the original payload if the needle cannot be found",
				(t: Test): void => {
		...
		t.test("2 (identical) character needle", (t: Test): void => {
			t.test(
				"should return the original payload if the needle cannot be found",
				(t: Test): void => {
		...
		t.test("empty payload", (t: Test): void => {
			t.test("should return nothing", (t: Test): void => {
				test(t, "1", split(""), [""], "");
				test(t, "abc", split(""), [""], "");
		...
		t.test("needles larger than the payload", (t: Test): void => {
			t.test("should return the original payload", (t: Test): void => {
		...
		t.test("misc", (t: Test): void => {
			t.test("should work", (t: Test): void => {
				test(t, "hello", split("hello world"), ["", " world"], "");
				test(t, "hello", split("helo world"), ["helo world"], "");
				test(
					t,
					"hello world!",
					split("oh my, hello world"),
					["oh my, hello world"],
					"hello world",
				);
				test(
					t,
					"hello world!",
					split("oh my, hello world!! again, hello world!!"),
					["oh my, ", "! again, ", "!"],
					"",
				);
...
t.test("constructor", (t: Test): void => {
	const token = "hello";

	t.test("should work with string", (t: Test): void => {
		t.doesNotThrow(() => new StreamSearch(token));

		t.end();
	});

	t.test("should work with Uint8Array", (t: Test): void => {
...
suite(t, "feeding data in chunks of 3 bytes", (s: string) =>
	Array.from({ length: Math.ceil(s.length / 3) }, (_, i) =>
		s.substring(i * 3, (i + 1) * 3),
	),
);
