...
async function testSuite(
	t: Test,
	makeIter: (needle: string, chunks: string[]) => AsyncIterable<Token>,
	needle: string,
	payload: string,
	expected: string[],
): Promise<void> {
	await Promise.all(
		[2, 3, 4, 5, 6].map(async (chunkSize: number) => {
			const chunks = Array.from(
				{ length: Math.ceil(payload.length / chunkSize) },
				(_: undefined, i: number) => payload.substr(i * chunkSize, chunkSize),
			);
			t.test(`chunk size ${chunkSize}`, async (t: Test): Promise<void> => {
				t.same(await allStrings(makeIter(needle, chunks)), expected);
...
t.test("readable", async (t: Test): Promise<void> => {
...
t.test("queueable", async (t: Test): Promise<void> => {
...
t.test("iterator", async (t: Test): Promise<void> => {
	function makeIter(needle: string, chunks: string[]): IteratorStreamSearch {
		return new IteratorStreamSearch(
			needle,
			(async function* () {
